...
- **Any active `CustomUiComponent` intercepts all `OnFocusedInteractionInputStartedEvent` taps** — including the tap-to-start and ball-release inputs. To hide a UI panel without blocking game controls, set `customUiComponent.isVisible = false` from the ViewModel component. Do **not** rely solely on XAML visibility bindings (`Visibility="Hidden"`, `Opacity="0"`, etc.): the XAML element may be invisible but the `CustomUiComponent` remains active and will swallow taps.
