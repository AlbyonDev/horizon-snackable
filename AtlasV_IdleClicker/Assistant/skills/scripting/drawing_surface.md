...
import { ImageBrush, Rectangle } from 'meta/worlds';

// Use directly with ImageBrush
const playerBrush = new ImageBrush(playerTexture);
this.builder.drawRect(playerBrush, null, new Rectangle(player.x, player.y, 32, 32));

// Or with drawImage
this.builder.drawImage(backgroundTexture, new Rectangle(0, 0, 480, 800));
```
...
this.builder.drawRect(redBrush, outlinePen, new Rectangle(x, y, width, height));
```
...
The DrawingCommandsBuilder and related types are part of the durable Drawing API, re-exported from the Worlds SDK at `meta/worlds`.
...
> - `GradientStop`, `DrawTextOptions`, `ImageBrushOptions`, `IRectangle`, `IVec2` - Interfaces
>
...
**Note:** `meta/worlds` (the `worlds_sdk` kit) re-exports the full drawing surface — `CustomUiComponent`, `uiViewModel`, `UiViewModel`, `UiEvent`, the Drawing API, and the platform primitives (`Color`, `Vec2`, `Rectangle`, etc.). Import everything from `meta/worlds` for Worlds SDK / 3P scripts. The Drawing API is no longer experimental — only `CustomUiApiExperimental.fetchElementBounds` and its `CustomUiExperimentalService` wrapper remain experimental.
...
> The drawing-specific text enums are named `DrawFlowDirection`, `DrawTextAlignment`, `DrawTextTrimming` to avoid colliding with the generic Noesis enums `FlowDirection`, `TextAlignment`, `TextTrimming` (which are not re-exported from `meta/worlds`).
...
`Brush.type` is now a numeric enum (formerly a string-literal union). Discriminate brushes by enum case rather than by string:
...
// Discriminating on brush.type
switch (brush.type) {
  case BrushType.Solid: /* ... */ break;
  case BrushType.LinearGradient: /* ... */ break;
  case BrushType.RadialGradient: /* ... */ break;
...
interface DrawTextOptions {
  flowDirection?: DrawFlowDirection;    // Default: LeftToRight
  lineHeight?: number;                  // Line height in pixels. 0 uses font default. Default: 0
  textAlignment?: DrawTextAlignment;    // Default: Left
  textTrimming?: DrawTextTrimming;      // Default: WordEllipsis
}
```
...
interface ImageBrushOptions {
  sourceRect?: IRectangle;     // Source rectangle for sprite sheets (e.g. new Rectangle(x, y, w, h) or {x, y, width, height})
...
const gradient = new LinearGradientBrush(
  new Vec2(0, 0),      // Start point
  new Vec2(0, 100),    // End point (vertical gradient)
  [
...
new RadialGradientBrush(
  center: IVec2,        // Center point
  radii: IVec2,         // Horizontal and vertical radii
  stops: GradientStop[],
  origin?: IVec2        // Gradient origin (defaults to center)
)
```
...
const radial = new RadialGradientBrush(
  new Vec2(100, 100),  // Center
  new Vec2(50, 50),    // Radii
  [
...
const highlight = new RadialGradientBrush(
  ...
  new Vec2(80, 80)     // Offset origin for highlight effect
);
```
...
this.builder.drawRect(textureBrush, null, new Rectangle(0, 0, 480, 800));
```

**Example - Sprite from sprite sheet:**
```typescript
const spriteBrush = new ImageBrush(spriteSheet, {
  sourceRect: new Rectangle(32, 0, 32, 32),  // Select frame from sheet
  stretch: Stretch.Uniform
});
this.builder.drawRect(spriteBrush, null, new Rectangle(enemy.x, enemy.y, 32, 32));
```
...
this.builder.drawRect(tiledBrush, null, new Rectangle(0, 0, 480, 800));
```
...
```typescript
drawLine(pen: Pen, start: IVec2, end: IVec2): void
```

**Example:**
```typescript
this.builder.drawLine(whitePen, new Vec2(0, 0), new Vec2(100, 100));
```
...
drawRect(
  ...
  rect: IRectangle         // Destination rectangle
): void
```
...
```typescript
// Filled rectangle
this.builder.drawRect(redBrush, null, new Rectangle(10, 10, 100, 50));

// Outlined rectangle
this.builder.drawRect(null, whitePen, new Rectangle(10, 10, 100, 50));

// Filled and outlined
this.builder.drawRect(redBrush, whitePen, new Rectangle(10, 10, 100, 50));

// Object-literal form also works
this.builder.drawRect(redBrush, null, {x: 10, y: 10, width: 100, height: 50});
```
...
drawRoundRect(
  ...
  pen: Pen | null,
  rect: IRectangle,        // Destination rectangle
  radii: IVec2             // Corner radii (x = horizontal radius, y = vertical radius)
): void
```

**Example:**
```typescript
this.builder.drawRoundRect(blueBrush, null, new Rectangle(10, 10, 100, 50), new Vec2(8, 8));
```
...
```typescript
// Circle (equal radii)
this.builder.drawEllipse(brush, null, new Vec2(100, 100), new Vec2(25, 25));

// Ellipse
this.builder.drawEllipse(brush, null, new Vec2(100, 100), new Vec2(50, 25));
```
...
drawText(
  text: string,
  rect: IRectangle,          // Bounding rectangle
...
this.builder.drawText('SCORE', new Rectangle(10, 10, 100, 30), 16, textBrush, font);
this.builder.drawText(String(score), new Rectangle(80, 10, 100, 30), 24, textBrush, font);
```
...
this.builder.drawText(
  'Line 1\nLine 2\nLine 3',
  new Rectangle(20, 100, 200, 150),
  14,
  bodyBrush,
  bodyFont,
  { lineHeight: 20, textTrimming: DrawTextTrimming.WordEllipsis }
);
```
...
// Draw scaled image
this.builder.drawImage(iconTexture, new Rectangle(10, 10, 32, 32));
```
...
const spriteBrush = new ImageBrush(spriteSheet, {
  sourceRect: new Rectangle(frameX, 0, 32, 32)
});
this.builder.drawRect(spriteBrush, null, new Rectangle(enemy.x - 16, enemy.y - 16, 32, 32));
```
...
const animBrush = new ImageBrush(walkAnimation, {
  sourceRect: new Rectangle(sourceX, 0, 32, 32)
});
this.builder.drawRect(animBrush, null, new Rectangle(player.x - 16, player.y - 16, 32, 32));
```
...
```typescript
pushTranslate(offset: IVec2): void
```

### Push Rotate

```typescript
pushRotate(angleDeg: number, center: IVec2): void
// angleDeg: rotation angle in degrees (clockwise)
// center: center of rotation (required)
```

### Push Scale

```typescript
pushScale(scale: IVec2, center: IVec2): void
// scale: scale factors (x = horizontal scale, y = vertical scale)
// center: center of scaling (required)
```
...
```typescript
this.builder.pushTranslate(new Vec2(shipX, shipY));
this.builder.pushRotate(shipAngle, new Vec2(0, 0));  // Rotate around origin (already translated)
this.builder.drawPath(fillBrush, outlinePen, shipPath);
...
for (const asteroid of this.asteroids) {
  this.builder.pushTranslate(new Vec2(asteroid.x, asteroid.y));
  this.builder.pushRotate(asteroid.angle, new Vec2(0, 0));
  this.builder.pushScale(new Vec2(asteroid.radius, asteroid.radius), new Vec2(0, 0));
...
private render(): void {
  ...
  for (const obj of this.objects) {
    this.builder.drawRect(this.bgBrush, this.outlinePen, new Rectangle(obj.x, obj.y, obj.w, obj.h));
  }
...
this.builder.pushTranslate(new Vec2(ship.x, ship.y));
this.builder.pushRotate(ship.rotation, new Vec2(0, 0));
...
private drawBullets(): void {
  ...
  for (const bullet of this.bullets) {
    ...
    const bulletBrush = new ImageBrush(bulletSheetTexture, {
      sourceRect: new Rectangle(frameX, 0, frameSize, frameSize)
    });

    this.builder.drawRect(
      bulletBrush,
      null,
      new Rectangle(bullet.x - 8, bullet.y - 8, 16, 16)
    );
  }
}
```
...
private drawPlayer(): void {
  ...
  this.builder.drawRect(
    playerBrush,
    null,
    new Rectangle(this.player.x - 16, this.player.y - 16, 32, 32)
  );
}
```
...
Types like Brush, GradientStop, DrawTextOptions, ImageBrushOptions, IRectangle, IVec2 REQUIRE import type syntax.

```typescript
import { DrawingCommandsBuilder, SolidBrush, Pen } from 'meta/worlds';
import type { Brush, GradientStop } from 'meta/worlds';
```
