...
```typescript
import {uiViewModel, UiViewModel, DrawingCommandData} from 'meta/worlds';
...
class MyGameViewModel extends UiViewModel {
  ...
  // Required by UiViewModel — declare an empty events bag if you don't have UiEvents
...
- `drawCommands` is a `DrawingCommandData` instance, bound to `<local:DrawingSurface Commands="{Binding drawCommands}" />`
...
The `meta/worlds` package exports `UiEvent` but NOT `UiCommand`. Use `UiEvent` for XAML button handling. See the **xaml_and_ui** skill for the complete UiEvent pattern.
