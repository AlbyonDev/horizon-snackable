...
private renderDrawing(): void {
  ...
  for (let i = 1; i < this.drawingPoints.length; i++) {
    ...
    this.builder.drawLine(pen, new Vec2(p0.x, p0.y), new Vec2(p1.x, p1.y));
  }
}
```
...
private renderJoystick(): void {
  ...
  this.builder.drawEllipse(ringBrush, null,
    new Vec2(this.joystickCenterX, this.joystickCenterY),
    new Vec2(this.kJoystickRadius, this.kJoystickRadius));
  ...
  this.builder.drawEllipse(knobBrush, null, new Vec2(knobX, knobY), new Vec2(25, 25));
}
...
private renderButtons(): void {
  for (const btn of this.buttons) {
    ...
    this.builder.drawRoundRect(bgBrush, null,
      new Rectangle(btn.x, btn.y, btn.width, btn.height), new Vec2(8, 8));
    ...
    this.builder.drawText(btn.label, new Rectangle(textX, textY, btn.width, 20), 18, textBrush, btnFont);
  }
}
