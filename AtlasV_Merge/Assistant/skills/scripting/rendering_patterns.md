...
private drawPlayerFromSheet(): void {
  ...
  this.builder.drawRect(
    playerBrush, null,
    new Rectangle(this.player.x - 16, this.player.y - 16, 32, 32)  // Centered at player position
  );
}
```
...
private animateSprite(): void {
  ...
  this.builder.drawRect(
    animBrush, null,
    new Rectangle(this.player.x, this.player.y, 32, 32)
  );
}
```
...
private drawBullets(): void {
  ...
  for (const bullet of this.bullets) {
    ...
    const bulletBrush = new ImageBrush(this.bulletSheet, {
      sourceRect: new Rectangle(sourceX, 0, bulletSize, bulletSize)
    });

    this.builder.drawRect(
      bulletBrush, null,
      new Rectangle(bullet.x - 8, bullet.y - 8, bulletSize, bulletSize)  // Centered
    );
  }
}
```
...
private drawRotatedSprite(): void {
  this.builder.pushTranslate(new Vec2(this.ship.x, this.ship.y));
  this.builder.pushRotate(this.ship.angle, new Vec2(0, 0));

  // Draw centered at origin (transformed to ship position)
  const shipBrush = new ImageBrush(this.shipSprite);
  this.builder.drawRect(
    shipBrush, null,
    new Rectangle(-16, -16, 32, 32)  // Offset to center
  );
...
private drawBackground(): void {
  ...
  this.builder.drawRect(bg, null, new Rectangle(0, 0, this.canvasWidth, this.canvasHeight));
}
```
...
private drawStars(): void {
  for (const star of this.stars) {
    ...
    this.builder.drawEllipse(brush, null, new Vec2(star.x, star.y), new Vec2(star.size, star.size));
  }
}
```
...
private drawBoard(): void {
  ...
  this.builder.drawRect(boardBg, null, new Rectangle(kBoardOffsetX, kBoardOffsetY, boardW, boardH));
  ...
  for (let col = 1; col < kBoardCols; col++) {
    ...
    this.builder.drawLine(gridPen, new Vec2(x, kBoardOffsetY), new Vec2(x, kBoardOffsetY + boardH));
  }
  for (let row = 1; row < kBoardRows; row++) {
    const y = kBoardOffsetY + row * kCellSize;
    this.builder.drawLine(gridPen, new Vec2(kBoardOffsetX, y), new Vec2(kBoardOffsetX + boardW, y));
  }
...
private drawCell(row: number, col: number, color: Color): void {
  ...
  this.builder.drawRect(brush, null,
    new Rectangle(x + inset, y + inset, kCellSize - inset * 2, kCellSize - inset * 2));
  ...
  this.builder.drawRect(highlightBrush, null,
    new Rectangle(x + inset, y + inset, kCellSize - inset * 2, 2));
  this.builder.drawRect(highlightBrush, null,
    new Rectangle(x + inset, y + inset, 2, kCellSize - inset * 2));
}
```
    ...
    for (let tx = 0; tx < this.mapWidth; tx++) {
      ...
      switch (tile) {
        case 0: // Empty/floor
          ...
          this.builder.drawRect(floorBrush, null, new Rectangle(x, y, kTileSize, kTileSize));
          break;
        case 1: // Wall
          const wallBrush = new SolidBrush(Color.fromHex('#4A4A4A'));
          this.builder.drawRect(wallBrush, null, new Rectangle(x, y, kTileSize, kTileSize));
          break;
        case 2: // Water
          const waterBrush = new SolidBrush(Color.fromHex('#3366AA'));
          this.builder.drawRect(waterBrush, null, new Rectangle(x, y, kTileSize, kTileSize));
          break;
      }
    }
  }
...
private drawShip(): void {
  ...
  this.builder.pushTranslate(new Vec2(this.shipX, this.shipY));
  this.builder.pushRotate(this.shipAngle, new Vec2(0, 0));
  ...
  if (this.thrusting) {
    ...
    const flameBrush = new LinearGradientBrush(
      new Vec2(0, 8), new Vec2(0, 8 + flicker),
      [
...
private drawAsteroids(): void {
  ...
  for (const a of this.asteroids) {
    this.builder.pushTranslate(new Vec2(a.x, a.y));
    this.builder.pushRotate(a.angle, new Vec2(0, 0));
    this.builder.pushScale(new Vec2(a.radius, a.radius), new Vec2(0, 0));
...
private drawPlayer(): void {
  ...
  this.builder.pushTranslate(new Vec2(this.playerX, this.playerY));
...
private drawParticles(): void {
  for (const p of this.particles) {
    ...
    this.builder.drawEllipse(brush, null, new Vec2(p.x, p.y), new Vec2(radius, radius));
  }
}
```
...
private drawClearAnimation(): void {
  ...
  for (const row of this.clearingLines) {
    ...
    const y = kBoardOffsetY + row * kCellSize;
    this.builder.drawRect(flashBrush, null, new Rectangle(x, y, kBoardCols * kCellSize, kCellSize));
  }
}
```
...
private drawHUD(): void {
  ...
  // Score - using drawText with bounding rectangle
  this.builder.drawText('SCORE', new Rectangle(10, 8, 70, 20), 14, labelBrush, hudFont);
  this.builder.drawText(String(this.score), new Rectangle(80, 8, 100, 24), 18, valueBrush, hudFont);

  // Level
  this.builder.drawText('LEVEL', new Rectangle(200, 8, 60, 20), 14, labelBrush, hudFont);
  this.builder.drawText(String(this.level), new Rectangle(260, 8, 50, 24), 18, valueBrush, hudFont);
}
```
...
private drawHealthBar(): void {
  ...
  this.builder.drawRect(bgBrush, null, new Rectangle(x, y, width, height));
  ...
  this.builder.drawRect(healthBrush, null, new Rectangle(x, y, width * healthPercent, height));

  // Border
  const borderBrush = new SolidBrush(Color.fromHex('#FFFFFF'));
  const borderPen = new Pen(borderBrush, 1);
  this.builder.drawRect(null, borderPen, new Rectangle(x, y, width, height));
}
```
...
private drawLives(): void {
  ...
  for (let i = 0; i < this.lives; i++) {
    this.builder.pushTranslate(new Vec2(420 + i * 20, 18));
    this.builder.pushScale(new Vec2(0.5, 0.5), new Vec2(0, 0));
  ...
  for (const block of blocks) {
    ...
    if (r >= 0) {
      ...
      this.builder.drawRect(ghostBrush, outlinePen,
        new Rectangle(x + 1, y + 1, kCellSize - 2, kCellSize - 2));
    }
  }
}
```
