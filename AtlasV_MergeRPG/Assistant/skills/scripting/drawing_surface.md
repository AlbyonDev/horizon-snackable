...
import { ImageBrush } from 'meta/custom_ui';
import { Rectangle } from 'meta/platform_api';
...
The DrawingCommandsBuilder and related types are exported from the durable `meta/custom_ui` module (re-exported through `meta/worlds` for Worlds SDK consumers).
...
**Note:** The `meta/custom_ui` module exports `CustomUiComponent`, `uiViewModel`, `UiViewModel`, `UiEvent`, the full procedural Drawing API (`DrawingCommandsBuilder`, `DrawingCommandData`, brush/pen/font classes, `BrushType`, `IRectangle`, `IVec2`, etc.), and the drawing-specific enums. Worlds SDK / 3P scripts import the same surface from `meta/worlds`.
...
// 2D point and rectangle interfaces accepted by drawing methods.
// `Vec2` and `Rectangle` from `meta/platform_api` are structurally
// assignable to these, so existing `new Vec2(...)` / `new Rectangle(...)`
// instances work unchanged. Plain object literals also work.
interface IVec2 {
  x: number;
  y: number;
}
...
`Brush.type` is exposed as the `BrushType` enum (numeric values are stable):
...
Use it in switch statements over `brush.type`:

```typescript
import { BrushType } from 'meta/custom_ui';
...
interface ImageBrushOptions {
  sourceRect?: IRectangle;     // Source rectangle for sprite sheets ({x, y, width, height})
...
new RadialGradientBrush(
  ...
  radii: IVec2,         // Horizontal and vertical radii (x = rx, y = ry)
  stops: GradientStop[],
...
```typescript
drawLine(pen: Pen, p1: IVec2, p2: IVec2): void
```
...
drawRect(
  ...
  rect: IRectangle         // Destination rectangle ({x, y, width, height})
): void
```
...
// Plain object literal also works
...
drawRoundRect(
  ...
  pen: Pen | null,
  rect: IRectangle,
  radii: IVec2              // Corner radii (x = rx, y = ry)
): void
```
...
drawEllipse(
  ...
  pen: Pen | null,
  center: IVec2,            // Center point
  radii: IVec2              // X and Y radii (x = rx, y = ry)
): void
```
...
drawText(
  text: string,
  rect: IRectangle,          // Bounding box ({x, y, width, height})
...
private drawBullets(): void {
  ...
  for (const bullet of this.bullets) {
    ...
    this.builder.drawRect(
      bulletBrush,
      null,
      new Rectangle(bullet.x - 8, bullet.y - 8, 16, 16)  // Destination (centered, 16x16)
    );
  }
}
```
...
```typescript
import { DrawingCommandsBuilder, SolidBrush, Pen } from 'meta/custom_ui';
import type { Brush, GradientStop } from 'meta/custom_ui';
```
