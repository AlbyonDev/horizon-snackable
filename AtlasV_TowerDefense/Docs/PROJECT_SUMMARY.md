...
Scripts/
  ...
  Defs/
    TowerDefs.ts    — TOWER_DEFS: ITowerDef[] (8 towers + upgrade trees)
    EnemyDefs.ts    — ENEMY_DEFS: IEnemyDef[] (15 enemy types including charger, yeti berserker, frost goblin, fire goblin, fire golem, cave boss, giant goblin, fireball, splitter, miniSplitter, and phaseShifter; each has a `followPath` property controlling path vs straight-line movement)
    ...
    SkillTreeDefs.ts — SKILL_NODES + SKILL_CONNECTIONS: explicit graph-based skill tree (1 root node + 33 branch nodes; 10 tiers × 3 branches + 3 extension nodes at indices 31–33: UnlockSniper, LightningRangeT1, PoisonDamageT1; directed edge connections define prerequisites; supports lateral cross-branch links)
  ...
  Components/
    ...
    GroundBiomeController — Swaps ground plane material on BiomeChanged event; toggles mesh visibility on GamePhaseChanged (visible during Build/Wave/WaveClear, hidden otherwise)
...
| `sniper` | Sniper | 180g | 80 | 50 (global) | 0.3/s | — | Long-range single-target, high damage, very slow fire rate; upgrades: damage or rate (T1), range+rate or damage+range (T2) |
...
| `splitter` | Splitter | 200 | 0.9/s | 12g | `splitOnDeath: { enemyId: 'miniSplitter', count: 3 }` — on death spawns 3 MiniSplitters at its path position that continue along the path. Only appears in run 2+ waves (via `minRun: 2` on wave groups). |
| `miniSplitter` | Mini Splitter | 40 | 2.0/s | 4g | Fast but fragile offspring spawned by the Splitter on death. Shares the Splitter mesh at 0.5× scale. |
| `phaseShifter` | Phase Shifter | 100 | 1.0/s | 10g | `blinkIntervalSec: 3`, `blinkCells: [2, 3]` — every 3 seconds, teleports forward 2-3 random cells along the path, skipping tower coverage. Brief shrink/grow visual telegraph on blink with purple particle bursts at disappear/reappear positions and dual SFX (whoosh on vanish, sparkly materialize on reappear). Punishes clustered tower placement, rewards spread placement. Appears in Tier 2 wave packs (PhaseShifterAmbush: 3 phase shifters + 3 basic). |
...
| Skill Tree | Permanent meta-progression purchased with skulls. 1 root node + 33 branch nodes (10 tiers × 3 branches + 3 extension nodes at indices 31–33) with directed graph connections (SKILL_CONNECTIONS edges), allowing lateral cross-branch links and flexible prerequisite paths. TOTAL_SKILLS = 34. A node is purchasable when at least one incoming-connected node is unlocked. Unlocks persist in `TdSaveData.st`. Opens from the overworld skull header tap. Bonuses: +damage, +fire rate, +crit, +lives, +range, +starting gold, +wave bonus, +sell refund, +poison damage, +lightning range. Special unlock nodes: Unlock Laser Canon (index 8), Unlock Poison Tower (index 10, cost 8 skulls), Unlock Snow Biome (index 14, cost 10 skulls), Unlock Fire Cannon for all biomes (index 16, cost 12 skulls), Unlock Lightning Tower (index 18, cost 14 skulls), Unlock Frost Tower for all biomes (index 19, cost 12 skulls), Unlock Volcano Biome (index 20, cost 18 skulls), Unlock Sniper Tower (index 31, cost 40 skulls, connected from node 28). Biome unlock nodes gate biome navigation arrows in the Overworld HUD. Extension nodes: Unlock Sniper Tower (index 31, cost 40 skulls, tag UnlockSniper, connected from node 28), Lightning Range Increase +30% (index 32, cost 30 skulls, tag LightningRangeT1, connected from node 29), Poison Damage Increase +25% (index 33, cost 30 skulls, tag PoisonDamageT1, connected from node 30). Nodes 24, 25, and 29 are infinitely re-purchasable: each purchase stacks the bonus additively, purchase counts persist in `TdSaveData.stc`, and these nodes display with a teal ring in the UI. |
...
- `GroundBiomeController` (on the ground Plane entity) swaps the MaterialComponent at runtime and toggles `MeshComponent.isVisibleSelf` based on the active `GamePhase` (visible during Build/Wave/WaveClear; hidden during Idle/Overworld/GameOver/Victory)
...
- `LifeLossSfxService` (on AudioManager entity) plays a non-spatial 2D one-shot punch impact SFX (`SFX_impact_punch_regular_01`) each time an enemy reaches the end of the path (life lost). Subscribes to `Events.EnemyReachedEnd`. Volume 0.7943 (loudness-normalised), pitch randomised ±5% per play. Asset: `SFX_impact_punch_regular_01/`.
- Audio assets: `Lava_Ambience/Lava_Ambience.ogg` (volcano ambient), `Wind_Cold_Low_Loop/Wind_Cold_Low_Loop.ogg` (snow ambient), `a_walk_in_the_forest_QZAAJ1902071/a_walk_in_the_forest_QZAAJ1902071.ogg` (grass overworld), `phantoms_fantasies_QZAAJ2000898/phantoms_fantasies_QZAAJ2000898.ogg` (wave music all biomes).
...
| **Skill Tree** | UI/SkillTree.xaml | Overworld (skull tap) | ✅ — Fullscreen overlay with hazy gradient background (dark muted purple → warm reddish-brown). Prominent root node at top with skull icon sprite. 3 branches × 10 tiers + 3 extension nodes (indices 31–33) of circular nodes with pale cream wavy bezier connecting lines. Node positions are hardcoded in XAML (no Canvas position bindings). Each node displays a thematic sprite icon representing its bonus category (sword=damage, heart=lives, lightning=fire rate, crosshair=crit, tower=unlock, range=tower range, coin=starting gold, treasure=wave bonus, refund=sell refund, skull=root). Icons stored in `sprites/skilltree/`. Cross-link paths between branches create a web-like interconnected feel. Nodes show unlocked (gold border), affordable (cream border), or locked (dim) states. Tower-unlock nodes use a two-column layout (card image left, name/description/cost right); non-tower nodes keep centered layout. Purchase deducts skulls. Fantasy font (Anton). Close button returns to overworld. |
