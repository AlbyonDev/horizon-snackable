...
| **Magma FTUE** | UI/MagmaFtue.xaml | Build (volcano biome, first time only) | ✅ — One-time fullscreen overlay popup explaining magma tiles block tower placement and scale with run count. Fiery gold title ("VOLCANIC HAZARD"), body text, orange "Got it!" dismiss button. Triggered on LevelSelected when biome is volcano and `vf` flag is not set. Persisted in save data (`vf` flag). |
